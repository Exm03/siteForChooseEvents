...
startBtn.addEventListener('click', function () {
    section.classList.add('slide-out-top')

    setTimeout(innerBox, 600)
...
function checkPlace(eventChange, eatsChange) {
    section.classList.remove('slide-in-bottom', 'slide-in-br', 'slide-in-left')
    bats = document.querySelectorAll('.bat');
    bats.forEach(btn => {
        btn.addEventListener('click', function (event) {
            section.classList.add('slide-out-bck-cente')
            placeChange = event.target.id
            
            setTimeout(animForSect, 600, eventChange, eatsChange, placeChange)
        })
    })

 }

function animForSect (eventChange, eatsChange, placeChange) {
    section.classList.remove('slide-out-bck-cente')
    
    section.classList.add('scale-in-center')
...
function checkEats(eventChange) {
    ...
    btns.forEach(btn => {
        btn.addEventListener('click', function (event) {
            ...
            section.classList.add('slide-out-tl')
            setTimeout(innerBoxEventsChek, 600, eventChange, eatsChange)
        })
    })
}

function innerBoxEventsChek (eventChange, eatsChange) {
    section.classList.remove('slide-out-tl')
    section.classList.add('slide-in-br')
...
function checkEvents() {
    ...
    newBats.forEach(btn => {
        btn.addEventListener('click', function (event) {
            ...
            setTimeout (innerBoxEvents, 600, eventChange)
    
        })
    }) 
}

function innerBoxEvents(eventChange) {
